...
exports.getTasks = async (req, res) => {
    try {
      const result = await pool.query("SELECT * FROM TASKS WHERE USER_ID = $1 AND deleted = false", [
        req.user.id,
      ]);
...
exports.updateTask = async (req, res) => {
  ...
  exports.deleteTask = async (req, res) => {
    try {
      const { id } = req.params;
      const result = await pool.query(
        "UPDATE TASKS SET deleted = true WHERE TASK_ID = $1 AND USER_ID = $2 RETURNING *",
        [id, req.user.id]
      );
      if (result.rows.length > 0) {
